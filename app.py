# app.py  – Esoteric Connect backend
...
# ──────────────────────────── конфигурация ────────────────────────────
postgres_url = (
    os.getenv("DATABASE_URL")              # если задали вручную
    or os.getenv("INTERNAL_DATABASE_URL")  # Render создаёт автоматически
)
if postgres_url and postgres_url.startswith("postgres://"):
    # SQLAlchemy ожидает postgresql+psycopg2://
    postgres_url = postgres_url.replace("postgres://", "postgresql+psycopg2://", 1)

app = Flask(__name__)
app.config.update(
    SECRET_KEY="changeme-secret-key",
    SQLALCHEMY_DATABASE_URI=postgres_url or "sqlite:///data.db",
...
class User(db.Model, UserMixin):
    ...
    def to_dict(self):
        return {
            ...
            "name": self.name, "photo": self.photo,
...
@login_required
def api_logout():
    logout_user(); return {"msg": "logged out"}
...
def api_update():
    ...
    d = request.get_json()
    for f in ("name", "specialty", "photo"):
        if f in d:
            setattr(current_user, f, d[f])
    db.session.commit()
    return {"msg": "updated", "user": current_user.to_dict()}

# ──────────────────────────── демо-мастера ──────────────────────────
def seed_demo():
    if User.query.filter_by(role="master").count() >= 3:
        return
    demo = [
        ("elena@demo",  "Elena Star",   "Astrologer",
         "https://randomuser.me/api/portraits/women/44.jpg"),
        ("maxim@demo",  "Maxim Aura",   "Tarot Reader",
         "https://randomuser.me/api/portraits/men/45.jpg"),
        ("natalie@demo","Natalie Numbers","Numerologist",
    ...
    for e, n, s, p in demo:
        ...
        db.session.add(User(
            ...
            role="master", name=n, specialty=s, photo=p
        ))
    db.session.commit()
